...
`/api/*` route locally. Add `OPENSTATES_API_KEY="..."` to `.dev.vars` first if you
want exact lawmaker lookup to return real offices in local Worker simulation.
...
bunx wrangler secret put OPENSTATES_API_KEY
...
- `worker.ts` — Cloudflare Worker API for persistent action logs and optional exact lawmaker lookup via Open States.
...
- Exact lawmaker lookup uses `navigator.geolocation` only after the visitor clicks the button. The Worker sends the coordinate to Open States `/people.geo`, returns public state-lawmaker names and phone numbers, and does not write location to D1. Without `OPENSTATES_API_KEY`, the UI falls back to the official state lookup link.
